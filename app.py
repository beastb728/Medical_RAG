...
def normalize_context(ctx: str) -> str:
    ...
    ctx = ctx.replace(",", " ")
    ctx = re.sub(r"\s+", " ", ctx)
    ctx = ctx.replace(" serum", " serum")
    ctx = ctx.replace("profileserum", "profile serum")
    return ctx.strip()
...
def normalize_unit(unit: str) -> str:
    ...
    u = u.replace("mg/dl", "mg/dl")
    u = u.replace("ratio", "ratio")
    return u.strip()


def comparison_key(test_name: str, test_context: str, unit: str) -> str:
    name = re.sub(r"\s+", " ", (test_name or "").lower()).strip()
    ctx = normalize_context(test_context)
...
if selected_patient:
    ...
    rows = conn.execute("""
        SELECT
            r.report_date,
            COALESCE(t.canonical_id, t.test_name) AS test_name,
            t.test_context,
            t.value,
            t.unit,
            t.normal_range
    ...
    """, (selected_patient.lower(),)).fetchall()
    conn.close()

    if not rows:
        st.warning("no test results found for this patient")
    else:
        df = pd.DataFrame(
            rows,
            columns=[
        ...
        df["display_test"] = df.apply(
            lambda r: (
                f"{r['test_name']} ({r['test_context']})"
                if r["test_context"]
                else r["test_name"]
            ),
            axis=1
        ...
        df = df[
            ["cmp_key", "date", "display_test", "result", "reference_range", "unit"]
        ]


        st.subheader("patient test results")
        st.dataframe(df, hide_index=True)
        st.subheader("comparison view (dates as columns)")
        ...
        meta = (
            df.groupby("cmp_key")[["display_test", "reference_range", "unit"]]
            .first()
        )
        ...
        date_cols = [
            ...
            if c not in ["display_test", "reference_range", "unit"]
        ]

        final_df = final_df[
            ["display_test"] + date_cols + ["reference_range", "unit"]
        ]
